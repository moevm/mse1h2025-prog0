...
* `is_simple_task` - значение логического типа, определяющее будет ли задача упрощенной или нет;
  * Сравнение обычного и упрощенного режима представлено в Таблице 1.
    ...
    | Различия в тексте задания     | 1. Требуется реализовать программу<br/>2. Входные данные подаются в `stdin`<br/>3. Выходные данные должны быть в `stdout` | 1. Требуется реализовать функцию<br/>2. Входные данные подаются в функцию как указатель `arr`<br/>3. Выходные данные отсутствуют                             |
